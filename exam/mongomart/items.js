...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */		
		this.db.collection("item").aggregate(
			...
			{$project:{_id:1,num:{$size:"$items"}} },
			function( err, categories ) {
				if ( err )
					throw err;
				var sum = categories.reduce((currentSum, b) => currentSum + b.num, 0);
				categories.push({ _id: "All",	num: sum });
				categories.sort((a, b) => (a._id > b._id) ? 1 : ((b._id > a._id) ? -1 : 0));
				callback(categories);
			}
		);
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
		...
		var sortCondition = category === "All"? {}:{"category" : category};
